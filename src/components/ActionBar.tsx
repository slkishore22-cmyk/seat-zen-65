  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 glass-card" style={{ borderRadius: 0, borderBottom: "none", borderLeft: "none", borderRight: "none" }}>
      <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-center gap-3 flex-wrap">
        <button className="btn-secondary text-sm" onClick={onReshuffle}>
        ...
        {showSave ? (
          <div className="flex items-center gap-2">
            <input
              ...
              autoFocus
            />
            <button className="btn-primary text-sm" onClick={handleSave} disabled={!roomName.trim()}>Save</button>
            <button className="btn-secondary text-sm" onClick={() => setShowSave(false)}>Cancel</button>
          </div>
        ) : (
          <button className="btn-secondary text-sm" onClick={() => setShowSave(true)}>
            <Save size={14} strokeWidth={1.5} className="mr-1.5" /> Save Room
          </button>
        )}

        <button className="btn-secondary text-sm" onClick={onNewRoom}>
          <Plus size={14} strokeWidth={1.5} className="mr-1.5" /> New Room
        </button>

        <button className="btn-secondary text-sm" onClick={onPrint}>
