...
const ActionBar = ({ onReshuffle, onSave, onNewRoom, onPrint, onHeightChange, inline = false }: Props) => {
  const [showSave, setShowSave] = useState(false);
  const [roomName, setRoomName] = useState("");
  const barRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    if (inline || !onHeightChange || !barRef.current) return;
